...
Alex and Hung prepared 1 mg of digested peptides (in solution digestion) and subjected 500ug to TiO2 enrichment. The samples prior and after phospho-enrichment were injected onto the Q-Exactive Plus in technical duplicate. We work with 3 biological replicates (independent transfections). 
...
```{r inputProt}
tab <- read.table("RAW/ProteomeMQ081217/proteinGroups.txt", sep = "\t", header = T, stringsAsFactors = F, quote = "")
```
...
#hist(tab$Unique.peptides[tab$Unique.peptides<=5], col = "darkblue", breaks = 6, main = "Unique peptides per protein - zoom in for less than 5")
```
...
```{r}
proteomes <- read.table("uniprotsmeg_20180116/uniprotsmeg_20180116_Simple.tab", sep = "\t", header = T, quote = "")
...
The vertical red line indicate the median intensity of PknB.
