...
vec2 <- as.character(IntProt$Entry.name[match(vec, as.character(IntProt$mc_155))])
...
tab <- cbind(tab, intnorm)
write.table(tab, "OutputTables/NormIntProt_20190127.txt", row.names = F, sep = "\t")
```
...
```{r}
# Remove the column with Excel issues (MQ IDs):
write.table(tab[,c(1:188,198:241)], "OutputTables/NormIntProt_20190127.txt", row.names = F, sep = "\t")
```
...
# I remove the values for the proteins only identified/quantified by phosphorylated peptides:
intnorm[tab$Only.identified.by.site == "+",] <- NA
...
write.table(tab, "OutputTables/NormIntPhospho_20190127.txt", row.names = F, sep = "\t")
```
...
```{r}
write.table(tab, "OutputTables/StatTable_20190127.txt", sep = "\t", row.names = F)
