...
# Order sites:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
int$Phosphosite <- factor(as.character(int$Phosphosite), levels = int$Phosphosite[order(tab$Intensity.K_Vs_Intensity.P_FC[tab$Significant_KVsP & tab$Significant_LVsP])]) # There are two entries with "mmpL5-T10"!! The sequences are different. Check what this is!!! Leading proteins are: I7G3R8 and I7GBH7
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
```{r}
keep <- ((toupper(tab$GeneID2) %in% toupper(c(as.character(IntProt$Entry.name), "pknb", "pknbtub", "mtpknb"))) | tab$GeneID2 == "MSMEG_4757")

# Select values:
int <- tab[keep,grepl("Intensity.K_Vs_Intensity.P_FC", names(tab))]
int <- data.frame("FC_KVsP"= int, "Phosphosite" = tab$PhosID[keep], stringsAsFactors = )
...
```{r}
IntProt <- read.table("OutputTables/ProteinsOfInterest2.txt", sep = "\t", header = T, stringsAsFactors = F)
